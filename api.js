...
exports.handler = async (event) => {
  ...
  const res = (code, data) => ({ statusCode: code, headers: CORS, body: JSON.stringify(data) });
  ...
  try {
    ...
    if (action === 'update') {
      ...
      const properties = {};
      if (status && statusLabel) properties['ステータス'] = { select: { name: statusLabel } };
      if (tags !== undefined) properties['タグ'] = { multi_select: tags.map(t => ({ name: t })) };
...
function mapFromNotion(page, type) {
  ...
  const roleMap = { 'ボランティア管理':'vol_mgmt','出店者サポート':'store_sup','企画・運営サポート':'planning' };
  const catMap  = { '飲食ブース':'food','キッチンカー':'truck','食材系・物販ブース':'goods','展示・PRブース':'exhibit' };
  const vSt = { '審査中':'pending','確定':'confirmed','補欠':'waitlist','不採用':'rejected' };
  const bSt = { '確認中':'pending','承認':'confirmed','補欠':'waitlist','不採用':'rejected' };

  if (type === 'booth') {
    return {
      id: page.id, type: 'booth',
      company:  title('社名・団体名'),
      shopname: text('店舗名・屋号'),
      name:     text('担当者名'),
      email:    email('メール'),
      phone:    phone('携帯番号'),
...
function volToNotion(d) {
  const roleLabel = { vol_mgmt:'ボランティア管理', store_sup:'出店者サポート', planning:'企画・運営サポート' };
  const props = {
    'お名前':     { title: [{ text: { content: d.name || '' } }] },
    'メール':     { email: d.email || null },
    '携帯番号':   { phone_number: d.phone || null },
...
function boothToNotion(d) {
  const catLabel = { food:'飲食ブース', truck:'キッチンカー', goods:'食材系・物販ブース', exhibit:'展示・PRブース' };
  const props = {
    '社名・団体名': { title: [{ text: { content: d.company || '' } }] },
    '店舗名・屋号': { rich_text: [{ text: { content: d.shopname || '' } }] },
